...
![](https://github.com/istiokrsg/istio_book_kr/tree/db537f7d53525991029377f8bdba34de236e0a89/.gitbook/assets/image-7.png)
...
쿠버네티스 default namespace 라벨링을 설정하여, Bookinfo 애플리케이션 배포시 자동으로 Envoy sidecar proxy를 설치하도록 합니다. 쿠버네티스 애플리케이션 배포시 배포할 네임스페이스를 설정할 수 있고, 여기서는 default namespace를 사용하도록 합니다. 

```text
kubectl label namespace default istio-injection=enabled
```

kubectl describe 명령어를 통해 default namespace에 라벨링이 정상적으로 적용되었는지 확인할 수 있습니다.

```text
kubectl descsribe ns default
...
이제 default namespace에 BookInfo 애플리케이션 배포시 각 pod에 자동으로 Envoy sidecar proxy가 설치될 것입니다.  다음 명령어를 통해 bookinfo 샘플 애플리케이션을 배포합니다. 설치가 성공적으로 끝나면 MSA 각 서비스 모듈 productpage, bookinfo-details, ratings, reviews 가 정상적으로 생성된 것을 확인할 수 있습니다.
...
#### 2. Bookinfo 애플리케이션 배포 확인

BookInfo 정상 배포 여부 확인을 위해 아래 명령어를 통해서 service, pod 상태 정보를 확인합니다.

```text
kubectl get services

NAME          TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)    AGE
details       ClusterIP   10.0.85.223    <none>        9080/TCP   13m
kubernetes    ClusterIP   10.0.0.1       <none>        443/TCP    22d
productpage   ClusterIP   10.0.111.103   <none>        9080/TCP   12m
ratings       ClusterIP   10.0.119.248   <none>        9080/TCP   13m
reviews       ClusterIP   10.0.134.246   <none>        9080/TCP   13m
```
...
NAME                             READY   STATUS    RESTARTS   AGE
details-v1-c5b5f496d-jx9f5       2/2     Running   0          14m
productpage-v1-c7765c886-k6tw8   2/2     Running   0          14m
ratings-v1-f745cf57b-ql5g9       2/2     Running   0          14m
reviews-v1-75b979578c-rwq2d      2/2     Running   0          14m
reviews-v2-597bf96c8f-mg8p8      2/2     Running   0          14m
reviews-v3-54c6c64795-fj9lw      2/2     Running   0          14m
test-shell-66dd9d74d6-kcrnb      1/1     Running   1          46h

```

#### 3. Bookinfo 애플리케이션 서비스 정상 동작여부 확인

아래 명령을 통해 쿠버네티스 클러스터내 rating pod 안에서 productpage로 curl 테스트를 통해 서비스 정상동작 여부를 확인할 수 있습니다. 정상적으로 수신한 경우 productpage 앱으로부터 받은 응답메시지에 grep 필터링을 통해 &lt;title&gt; 태크 값 "Simple Booksotre App" 문자열을 확인할 수 있습니다. 
